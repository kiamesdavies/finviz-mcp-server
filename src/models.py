...
@dataclass
class StockData:
    """Main model for stock data (covers all Finviz fields)."""
    ticker: str
    company_name: str
    sector: str
    industry: str
    ...
    # New: index and classification info
    index: Optional[str] = None  # Index membership (e.g., S&P 500)
    ...
    # New: short-interval performance
    ...
    performance_since_inception: Optional[float] = None  # New: since inception performance
    ...
    tags: Optional[str] = None  # New: tag info
...
class EarningsData:
    ...
    # Earnings results and surprises
    eps_surprise: Optional[float] = None
    revenue_surprise: Optional[float] = None
    eps_estimate: Optional[float] = None
    eps_actual: Optional[float] = None
...
# Old mapping constants were removed and merged into constants.py

# Finviz field mapping constants (keep the existing simple version)
