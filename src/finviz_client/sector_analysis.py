...
class FinvizSectorAnalysisClient(FinvizClient):
    """Client for Finviz sector/industry analysis."""
    ...
    def get_sector_performance(self, timeframe: str = "1d", 
        ...
        """
        Sector performance analysis (via CSV export).

        Args:
            timeframe: Timeframe (1d, 1w, 1m, 3m, 6m, 1y) - currently ignored
            sectors: Target sectors (all if None)

        Returns:
            List of SectorPerformance objects
        """
        try:
            # Fetch basic sector data (add v=152 parameter)
            params = {
                'g': 'sector',
                'v': '152'  # Correct view format
            }
            
            # Add API key (same handling as base.py)
            if self.api_key:
            ...
            # Convert CSV data to SectorPerformance objects
            sector_data = []
    ...
    def get_industry_performance(self, industries: Optional[List[str]] = None) -> List[Dict[str, Any]]:
        """
        Industry performance analysis (via CSV export).

        Args:
            industries: Target industries (all if None)
        ...
        try:
            ...
            # Convert CSV data to industry performance list
            industry_data = []
            for _, row in df.iterrows():
                try:
    ...
    def get_country_performance(self, countries: Optional[List[str]] = None) -> List[Dict[str, Any]]:
        """
        Country market performance analysis (via CSV export).

        Args:
            countries: Target countries (all if None)
        ...
        try:
            ...
            # Convert CSV data to country performance list
            country_data = []
            for _, row in df.iterrows():
                try:
    ...
    def get_sector_specific_industry_performance(self, sector: str) -> List[Dict[str, Any]]:
        """
        Industry performance within a specific sector.
    ...
    def get_capitalization_performance(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Convert CSV data to market cap performance list
            cap_data = []
            for _, row in df.iterrows():
                try:
    ...
    def _parse_sector_performance_from_csv(self, row: 'pd.Series') -> Optional[Dict[str, Any]]:
        """
        Build sector performance data from a CSV row.
    ...
    def _parse_industry_performance_from_csv(self, row: 'pd.Series') -> Optional[Dict[str, Any]]:
        """
        Build industry performance data from a CSV row.

        Args:
            row: pandas Series (CSV row data)

        Returns:
            Industry performance dict or None
        """
        try:
            import pandas as pd
    ...
    def _parse_country_performance_from_csv(self, row: 'pd.Series') -> Optional[Dict[str, Any]]:
        """
        Build country performance data from a CSV row.

        Args:
            row: pandas Series (CSV row data)

        Returns:
            Country performance dict or None
        """
        try:
            import pandas as pd
    ...
    def _parse_capitalization_performance_from_csv(self, row: 'pd.Series') -> Optional[Dict[str, Any]]:
        """
        Build market cap performance data from a CSV row.

        Args:
            row: pandas Series (CSV row data)

        Returns:
            Market cap performance dict or None
        """
        try:
            import pandas as pd
